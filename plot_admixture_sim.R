...
setwd("~/Dropbox (Personal)/publications/Ctenotus_zasticus/data/admixture_sim/")
sampfile = "../Ctenotus_zastictus.project3.csv"

d = read.csv(sampfile, stringsAsFactors = F)

pdf("~/Desktop/test.pdf", width = 10, height = 4)
files = list.files(pattern = "*.Q", full.names = T)
files = files[grep("\\.1.Q", files, invert = T)]
for (file in files) {
  k = read.table(file, stringsAsFactors = F)
  name = gsub(".*MAC2.", "", file)
  name = gsub(".Q", "", name)
  famfile = gsub("\\d.Q", "fam", file)
  inds = read.table(famfile)
  inds = data.frame(inds = inds$V2, 
                    sp = d[match(inds$V2, d$SAMPLE_ID), "nDNA_LINEAGE"], 
                    stringsAsFactors = F)
  inds[grep("zast", inds$inds), "sp"] = "zastictus"
